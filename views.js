...
const AI_TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: "crear_tp",
      description: "Crea un trabajo práctico (TP) con fecha límite para una materia.",
      parameters: {
        type: "object",
        properties: {
          materia: { type: "string", description: "Nombre de la materia (existente o nueva)" },
          titulo: { type: "string", description: "Título del TP, ej: TP1 — Recursividad" },
          fecha_limite: { type: "string", description: "Fecha límite en formato YYYY-MM-DD" }
        },
        required: ["materia", "titulo", "fecha_limite"]
      }
    }
  },
  {
    ...
    function: {
      name: "actualizar_tp",
      description: "Marca un TP como entregado/pendiente y opcionalmente le carga una nota, buscándolo por texto del título.",
      parameters: {
        type: "object",
        properties: {
          titulo_busqueda: { type: "string", description: "Texto para encontrar el TP por su título" },
          estado: { type: "string", enum: ["pendiente","entregado"] },
          nota: { type: "number", description: "Nota obtenida (opcional)" }
        },
        required: ["titulo_busqueda"]
      }
    }
  },
  {
...
function ejecutarHerramientaIA(name, args){
  try{
    ...
    if(name === "crear_tp"){
      ...
      const tp = {id:uid(), materiaId:m.id, titulo:args.titulo, fechaLimite:args.fecha_limite, estado:'pendiente', nota:null};
      db.tps.push(tp);
      saveDB();
      return { ok:true, mensaje:`TP "${args.titulo}" agregado a ${m.nombre} con fecha límite ${args.fecha_limite}.` };
    }

    if(name === "actualizar_tp"){
      const busqueda = args.titulo_busqueda.toLowerCase();
      const tp = db.tps.find(t=>t.titulo.toLowerCase().includes(busqueda));
      if(!tp) return { ok:false, mensaje:`No encontré ningún TP que coincida con "${args.titulo_busqueda}".` };
      if(args.estado) tp.estado = args.estado;
      if(typeof args.nota === 'number'){
        tp.nota = args.nota;
        tp.estado = 'entregado';
      }
      saveDB();
      return { ok:true, mensaje:`TP "${tp.titulo}" actualizado${tp.nota!==null?` (nota: ${tp.nota})`:''}.` };
    }
    ...
    if(name === "obtener_contexto_actual"){
      ...
      const tpsPendientes = db.tps.filter(t=>t.estado!=='entregado')
        .sort((a,b)=>a.fechaLimite.localeCompare(b.fechaLimite))
        .map(t=>{
          const m = materiaById(t.materiaId);
          return `${m?m.nombre:'?'}: ${t.titulo} (vence ${t.fechaLimite}, faltan ${daysUntil(t.fechaLimite)}d)`;
        });
...
Podés CREAR Y MODIFICAR datos reales del usuario (materias, lecturas, parciales, TPs, tareas) usando las herramientas disponibles. Usalas con confianza cuando el usuario pida crear/agregar/marcar algo, no le preguntes confirmación de más.
Los TPs (trabajos prácticos) son entregas con fecha límite, nota y estado pendiente/entregado. Suelen ser 4 por materia y entregarlos es lo que permite promocionar (evitar el final). Cuando el usuario pregunte por su situación de promoción, contá cuántos TPs entregó sobre el total de esa materia.
Cuando te pidan ayuda para PLANIFICAR el estudio, primero llamá a obtener_contexto_actual para tener datos reales (parciales próximos, lecturas pendientes, TPs pendientes), y armá un plan concreto y realista basado en eso, no genérico.
